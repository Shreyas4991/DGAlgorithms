...
structure SimplePN (V : Type u) extends PN_Net V where
  ...
  simple : ∀ v w : V, ∀ i : Fin (deg v), ∀ j : Fin (deg w), pmap ⟨v,i⟩ = ⟨w,j⟩ → v ≠ w
...
structure Algorithm (I S M: Type) where
  ...
  stopping_condition : ∀ y : M, ∀ s : S, s ∈ stopStates → recv (s, fun _ => y) = s
...
abbrev initState (N : SimplePN V) (A : Algorithm I S M) (inp : V → I) : AlgoState N S M where
  s_vec := fun v => A.init (inp v)
  msg_vec := fun v port => A.send (A.init <| inp v) port
  round := 0

abbrev updateState (N : SimplePN V) (A : Algorithm I S M) (cS : AlgoState N S M) : AlgoState N S M :=
  let new_s_vec := fun v => A.recv (cS.s_vec v, cS.msg_vec v)
  {
    s_vec := fun v => new_s_vec v
    msg_vec := fun v port => A.send (new_s_vec v) port
    round := cS.round + 1
  }

def terminated (A : Algorithm I S M) (N : SimplePN V) (state : AlgoState N S M): Prop :=
  ∀ v : V, state.s_vec v ∈ A.stopStates
